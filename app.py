...
def close_db(error):
    if hasattr(g, 'postgress_db_cur'):
        g.postgress_db_cur.close()
...
def get_current_user():
    ...
    if 'user' in session:
        ...
        db = get_db()  
        db.execute('select id, name, password, expert, admin from usersfile where name = %s', (user, ))
...
def index():
    ...
    db.execute('''select 
                ...
                from questions 
                join usersfile as askers on askers.id = questions.asked_by_id 
                join usersfile as experts on experts.id = questions.expert_id 
...
def register():
    ...
    if request.method == 'POST':
        db = get_db()
        db.execute('select id from usersfile where name = %s', (request.form['name'], ))
        existing_user = db.fetchone()

        if existing_user:
            return render_template('register.html', user=user, error='User already exists!')
        hashed_password = generate_password_hash(request.form['password'], method='sha256')
        db.execute('insert into usersfile (name, password, expert, admin) values (%s, %s, %s, %s)', (request.form['name'], hashed_password, '0', '0', ))
...
def login():
    ...
    if request.method == 'POST':
        ...
        db.execute('select id, name, password from usersfile where name = %s', (name, ))
        user_result = db.fetchone()
...
def question(question_id):
    ...
    db.execute('''select 
                questions.question_text, questions.answer_text, askers.name as asker_name, experts.name as expert_name 
                from questions 
                join usersfile as askers on askers.id = questions.asked_by_id 
                join usersfile as experts on experts.id = questions.expert_id 
...
def ask():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('index'))
    db.execute('select id, name from usersfile where expert = True')
...
def users():
    ...
    db = get_db()
    db.execute('select id, name, expert, admin from usersfile')
...
def promote(user_id):
    ...
    db = get_db()
    db.execute('update usersfile set expert = True where id = %s', (user_id, ))
